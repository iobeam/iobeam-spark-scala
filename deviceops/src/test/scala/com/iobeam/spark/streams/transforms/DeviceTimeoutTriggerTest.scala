...
class DeviceTimeoutTriggerTest extends FlatSpec with Matchers with MockitoSugar {
    val triggerEvent = "device_offline"
    ...
    "Threshold" should "trigger when no data for timeout period" in {
        val deviceTimeoutTrigger = (new DeviceTimeoutTrigger(timeout, triggerEvent)).getNewTransform
        ...
        deviceTimeoutTrigger.recordUpdateAndTest(new TestTimeRecord(timeoutUs, 2), 200) should
            equal(None)
        deviceTimeoutTrigger.batchDoneUpdateAndTest(timeoutUs + 1) should equal(None)
    ...
    "Threshold" should "trigger only once" in {
        ...
        val trigger = new DeviceTimeoutTrigger(timeout, triggerEvent).getNewTransform
